...
class SummaryChain(BaseChatChain):
    ...
    def run(self, full_text=None, chat_history=[]):
        return super().run(context=full_text, chat_history=chat_history)
    
    def parse_input(self, context, chat_history=[]):
        ...
        if len(encoded_text) > self.max_token_num:
            ...
            return {'context': decoded_text, 'chat_history': chat_history}
        else:
